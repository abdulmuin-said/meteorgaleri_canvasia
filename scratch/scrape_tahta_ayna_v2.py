...
def scrape_product(url):
    ...
    if not images and data.get('mainProductImage'):
        images.append(data.get('mainProductImage'))

    # Determine product type
    product_type = "Cam Kesme Tahtası" if "kesme-tahtasi" in url or "kesme-tahta" in url else "Ayna" if "ayna" in url else "Cam Tablo"
    ...
    if products and len(products) > 0:
        ...
        if variants_data:
            for i, vd in enumerate(variants_data):
                if not vd.get('aktif', True): continue
                vid = vd.get('urunID')
                tanim = vd.get('tanim', '')
        ...
        # Singular product without variants
        try:
            fiyat = float(data.get('productPrice', 0))
            if fiyat == 0:
                f_str = data.get('productPriceStr', '0').replace(',', '.')
                fiyat = float(f_str) if f_str else 0
        except:
            fiyat = 0
        
        # If still 0, try to find in HTML
        if fiyat == 0:
            m = re.search(r'"satisFiyati"\s*:\s*([^,]+)', html)
            if m:
                try:
                    fiyat = float(m.group(1))
                    fiyat = fiyat * 1.2 # KDV
                except:
                    pass

        variant_info.append({'olcu': 'Standart', 'fiyat': round(fiyat, 2), 'is_default': True})
